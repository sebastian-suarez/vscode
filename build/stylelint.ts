...
import { validateCodiconFontSizes, validateFontSizeTokens, validateCornerRadiusTokens, validateSpacingTokens, validateStrokeTokens } from './lib/stylelint/validateDesignTokens.ts';
 ...
 * `designTokensEverywhere` is `true`, i.e. when the caller explicitly targets a
 * path so the checks follow the requested scope.
 */
export default function gulpstylelint(reporter: Reporter, designTokensEverywhere = false): NodeJS.ReadWriteStream {
	...
	const designTokenCounts: Record<string, number> = { codicon: 0, 'font-size': 0, radius: 0, spacing: 0, stroke: 0 };
	...
	return es.through(function (this, file: FileWithLines) {
		...
		// They only run when `designTokensEverywhere` is set (an explicit path was
		...
		const designTokenTotal = designTokenCounts.codicon + designTokenCounts['font-size'] + designTokenCounts.radius + designTokenCounts.spacing + designTokenCounts.stroke;
 ...
 * explicit path was given (used to enable the design-token checks for the
 * requested path).
 */
