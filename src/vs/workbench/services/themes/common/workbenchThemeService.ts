...
export namespace ThemeSettingDefaults {
	...
	export const FILE_ICON_THEME = 'vsebcode-icons';
...
/**
 * The palette the workbench paints its first frame with, before the default dark theme has
 * been read off disk, so that a profile without a stored theme does not flash a foreign one.
 * Source of truth: `extensions/theme-defaults/themes/2026-dark.json` resolved over its
 * include chain — dark_vs.json, dark_plus.json, dark_modern.json, 2026-dark.json — with the
 * last definition of an id winning. Re-derive it from that chain whenever the theme moves.
 *
 * The surfaces that turn translucent over the macOS vibrancy are written opaque here: the
 * map is only the color source, and every consumer reads it back through
 * `ColorThemeData.getColor`, which is where the alpha is applied.
 */
