...
class EmptyTextEditorHintContentWidget extends Disposable implements IContentWidget {
	...
	private getHint() {
		...
		const changeLanguageKeybindingLabel = this.keybindingService.lookupKeybinding(ChangeLanguageAction.ID)?.getLabel();
		...
		}, '[[Select a language]] ({0}) to get started. Start typing to dismiss or [[don\'t show]] this again.', changeLanguageKeybindingLabel ?? '').replaceAll(' ()', '');
		...
		const ariaLabel = localize('defaultHintAriaLabel', 'Execute {0} to select a language and get started. Start typing to dismiss.', changeLanguageKeybindingLabel ?? '');
